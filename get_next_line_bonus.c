...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/12/18 03:22:04 by rzarquon          #+#    #+#             */
/*   Updated: 2021/12/18 05:17:20 by rzarquon         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	if (fd < 0 || BUFFER_SIZE < 1 || fd > OPEN_MAX)
		return (NULL);
	reminder[fd] = first_cut(fd, reminder[fd]);
	if (!reminder[fd])
		return (NULL);
	line = end_of_line(reminder[fd]);
	reminder[fd] = new_reminder(reminder[fd]);
	return (line);
}
